...
- [#7759](https://github.com/assistant-ui/assistant-ui/pull/7759) [`9ec1143`](https://github.com/assistant-ui/assistant-ui/commit/9ec1143bd892f9559251fb968d90c9d2f50f3c79) - fix: fold the prose a turn spoke before a parentless tool call onto that call's container, so a reloaded thread renders the turn as the one message the live run did ([@okisdev](https://github.com/okisdev))

- [#8139](https://github.com/assistant-ui/assistant-ui/pull/8139) [`18e1808`](https://github.com/assistant-ui/assistant-ui/commit/18e180856afd7f03e6c8f89e7d24b5c00a054abf) - fix: send an attachment whose data URL is not base64 (such as `data:text/plain,hello` or a percent-encoded SVG) as an AG-UI `url` source instead of a `data` source that claims to hold base64, so the agent receives the file intact and it is no longer wrapped in a second data URL when a snapshot is restored. ([@samdickson22](https://github.com/samdickson22))

- [#8282](https://github.com/assistant-ui/assistant-ui/pull/8282) [`03eb54c`](https://github.com/assistant-ui/assistant-ui/commit/03eb54c577978bddf51429af93cd82b2efa9460b) - fix(react-ag-ui): keep a finished answer complete when Stop lands before the response closes ([@samdickson22](https://github.com/samdickson22))

- [#7948](https://github.com/assistant-ui/assistant-ui/pull/7948) [`d6eba42`](https://github.com/assistant-ui/assistant-ui/commit/d6eba42413cd102acdccb7bdccac04bfb94ffc66) - feat(react-ag-ui): keep finalized voice transcripts in the thread, send them with the next run, and store them through a configured history adapter ([@okisdev](https://github.com/okisdev))

- [#8063](https://github.com/assistant-ui/assistant-ui/pull/8063) [`3eb6e06`](https://github.com/assistant-ui/assistant-ui/commit/3eb6e06f4cb52f7bfdfa94da363a00faa219f3ed) - fix: preserve unrendered activity snapshots as scoped `agui-activity/<type>` data parts across live runs and restored transcripts, keep a2ui surface rebuild operations in `artifact.a2ui`, and persist each run's final agent state on its settled assistant message ([@okisdev](https://github.com/okisdev))
...
- [#8067](https://github.com/assistant-ui/assistant-ui/pull/8067) [`9602712`](https://github.com/assistant-ui/assistant-ui/commit/9602712ba5bf98e319ba406b19bbf06f978d0a7e) - feat: the AG-UI runtime records tool interactions on their tool call, keeps them while the run streams and A2UI surfaces are rebuilt, and writes them to history ([@okisdev](https://github.com/okisdev))
